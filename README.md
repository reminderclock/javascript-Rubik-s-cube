...
3. 큐브 이동시켜 조작어 입력 받는 부분 `setData()`
...
15. 모두 맞추었을 시, 축하메세지 출력할 `addMessage()`
16. 무작위로 큐브를 섞어줄 `main.makeRandom()`

### 구현한 기능

1. 조작어를 `한글자`, 또는 `문자열`로 받아 큐브를 조작할수 있다.
2. `Q`를 눌러 게임을 `종료`할수 있다.
3. `무작위` 섞기로 큐브를 섞을 수있다.
4. 큐브를 다 맞추었을 경우, 종료시 `축하메세지`를 출력한다.
5. 종료시 `조작개수`, `종료시간`, `종료인사`을 출력한다.
